...
package org.blch.findPath
{
	...
	public class Cell extends Triangle
	{
		...
		private function checkPoint( p:Vector2f ):void
		{
			var mapVO:MapVO =  MapDataModel.instance.currentMapVO;
			if( p.x<=mapVO.tileWidth){
				p.x = mapVO.tileWidth ;
			}
			if( p.y<=mapVO.tileHeight){
				p.y = mapVO.tileHeight ;
			}
			if( p.x>=mapVO.mapWidth-mapVO.tileWidth){
				p.x = mapVO.tileWidth-mapVO.tileWidth ;
			}
			if( p.y>=mapVO.mapHeight-mapVO.tileHeight){
				p.y = mapVO.tileHeight-mapVO.tileHeight ;
			}
		}
