package  bing.starling.iso.path
{
	...
	public class SGrid
	{
		 ...
		 * Constructor.
		 */
		public function SGrid(numCols:int, numRows:int)
		{
			_numCols = numCols;
			_numRows = numRows;
			_nodes = new Array();
		 ...
		 * @param y The y coord.
		 */
		public function getNode(x:int, y:int):SNode
		{
			return _nodes[x][y] as SNode;
		}
		...
		{
			_endNode = _nodes[x][y] as SNode;
		}
		
		/**
		 * get nodes By Walkable
		 * @return SNode Vecter Array
		 */		
		public function getNodesByWalkable( walkable:Boolean ):Vector.<SNode>
		{
			var vec:Vector.<SNode> = new Vector.<SNode>();
			var node:SNode ;
			for(var i:int = 0; i < _numCols; i++)
			{
				for(var j:int = 0; j < _numRows; j++)
		...
		{
			_startNode = _nodes[x][y] as SNode;
		}
		 ...
		 */
		public function get endNode():SNode
		{
			return _endNode;
		}
		 ...
		 */
		public function get startNode():SNode
		{
			return _startNode;
		}
		 ...
		 *复制 
		 */		
		public function clone():SGrid
		{
			var gird:SGrid = new SGrid( this._numCols  , this._numRows );
			for(var i:int = 0; i < _numCols; i++)
			{
				for(var j:int = 0; j < _numRows; j++)
				{
