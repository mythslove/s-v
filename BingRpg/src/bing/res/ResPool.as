package bing.res
{
	...
	public class ResPool extends EventDispatcher
	{
		...
		protected var _resObject:ResVO ;
		 ...
		 */		
		public function queueLoad( resArray:Vector.<ResVO> ):void
		{
		...
		{
			_resObject = _resArray.shift();
			this.addEventListener( _resObject.name , resObjectLoadedHandler );
			this.loadRes( _resObject );
		}
		protected function resObjectLoadedHandler(e:Event):void
		{
			this.removeEventListener( _resObject.name , resObjectLoadedHandler );
			//加载进度
			var evt:ResProgressEvent = new ResProgressEvent(ResProgressEvent.RES_LOAD_PROGRESS);
			evt.total = _total ;
			evt.loaded = _total-_resArray.length ;
			evt.name = _resObject.name ;
